package com.travelme.customer.ui.main.home.destination_other_fragment
...
import com.travelme.customer.ui.hour.HourActivity
...
class DestinationOtherFragment : Fragment(R.layout.fragment_destination_other){
    ...
    private fun observer() = destinationOtherViewModel.listenToState().observer(requireActivity(), Observer {handleUI(it)})
    private fun observe() = destinationOtherViewModel.listenToDestinations().observe(requireActivity(), Observer { handleData(it) })
    ...
    private fun handleUI(it : DestinationOtherState){
        activity?.let { _ ->
            when(it){
                ...
                is DestinationOtherState.IsLoading -> handleLoading(it.state)
            }
        }
    }
