...
class DestinationTegalFragment : Fragment(R.layout.fragment_destination_tegal){
    ...
    private fun observer() = destinationTegalViewModel.listenToState().observer(requireActivity(), Observer {handleUI(it)})
    private fun observe() = destinationTegalViewModel.listenToDestinations().observe(requireActivity(), Observer { handleData(it) })
